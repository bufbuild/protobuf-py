...
"""A synchronous gRPC client for the Eliza service."""
...
from example_grpc.gen.connectrpc.eliza.v1.eliza_pb_grpc import ElizaServiceClientSync
...
def main(target: str = "localhost:50051") -> None:
    ...
    with grpc.insecure_channel(target) as channel:
        ...
        def requests() -> Iterator[ConverseRequest]:
            for sentence in conversation:
                print(f"Me: {sentence}")  # noqa: T201
                yield ConverseRequest(sentence=sentence)

        for response in client.converse(requests()):
            print(f"    Eliza: {response.sentence}")  # noqa: T201
